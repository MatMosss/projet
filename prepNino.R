recodage<-function(data){
    ...
    listi = as.list(unique(data$descr_etat_surf))
    i= 0 
      for (elt in listi){
        data$descr_etat_surf[data$descr_etat_surf==elt] <- i;
        i = i + 1 

    }
    listn = as.list(unique(data$descr_intersection))
    n= 0 
    for (elt in listn){
        data$descr_intersection[data$descr_intersection==elt] <- n;
         n = n + 1 

    }
       listm = as.list(unique(data$descr_intersection))
    n= 0 
    for (elt in listn){
        data$descr_intersection[data$descr_intersection==elt] <- n;
         n = n + 1 
...
recodage(read.csv2('stat_Ninho.csv'))